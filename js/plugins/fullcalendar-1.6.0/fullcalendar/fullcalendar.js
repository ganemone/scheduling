...
function Calendar(element, options, eventSources) {
	...
	function option(name, value) {
		...
		if (name == 'height' || name == 'contentHeight' || name == 'aspectRatio' || name == 'sorting') {
			options[name] = value;
			updateSize();
		}
	}
...
function segCmp(a, b) {
   return (b.msLength - a.msLength) * 100 + (a.event.start - b.event.start); // Original Sort
}

function segCmp_id(a, b) {
   return a.event.employeeId - b.event.employeeId;  // Sorts by employeeId
}

function segCmp_fname(a, b) {
   return (a.event.title > b.event.title) ? 1 : 0;  // Sorts alphabetically
}
...
function sliceSegs(events, visEventEnds, start, end, sorting) {
	var segs = [],
	...
	if(sorting == 'employeeId')
   	return segs.sort(segCmp_id);
   else if(sorting == 'firstName')
      return segs.sort(segCmp_fname);
   else
...
function BasicEventRenderer() {
	...
	function compileSegs(events) {
		var rowCnt = getRowCnt(),
			...
			segs=[];
		for (i=0; i<rowCnt; i++) {
			row = stackSegs(sliceSegs(events, visEventsEnds, d1, d2, opt('sorting')));
...
function AgendaEventRenderer() {
	...
	function compileDaySegs(events) {
		var levels = stackSegs(sliceSegs(events, $.map(events, exclEndDay), t.visStart, t.visEnd, opt('sorting'))),
	...
	function compileSlotSegs(events) {
		var colCnt = getColCnt(),
			...
			segs=[];
		for (i=0; i<colCnt; i++) {
			col = stackSegs(sliceSegs(events, visEventEnds, d, addMinutes(cloneDate(d), maxMinute-minMinute), opt('sorting')));
